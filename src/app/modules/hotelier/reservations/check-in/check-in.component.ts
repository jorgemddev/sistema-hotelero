...
export class CheckInComponent implements OnInit, OnChanges {
  ...
  form = new UntypedFormGroup({
    ...
    hour_ing: new UntypedFormControl(),
    check_ing: new UntypedFormControl(),
  ...
  getReservation(id: number) {
    this.api.getReservation(id).subscribe(
      (response) => {
        ...
        this.form.get('check_ing')?.setValue(this.helps.date()?.toISOString().substring(0, 10));
        const hour = this.helps.date()?.getHours().toString().padStart(2, '0');
        const minute = this.helps.date()?.getMinutes().toString().padStart(2, '0');
        this.form.get('hour_ing')?.setValue(hour + ":" + minute);
        this.calculateDayLate();
        if (this.reservations?.check_out || this.reservations?.check_ing ) {
          this.router.navigate(['hotel/habitaciones/check-out/', this.reservations_id]);
        }
      },
      (error) => {
